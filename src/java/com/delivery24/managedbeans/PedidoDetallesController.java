 ...
 * @author geovanny
 */
@Named(value = "pedidoDetallesController")
@SessionScoped
public class PedidoDetallesController implements Serializable 
{
    ...
    public PedidoDetallesController() 
    {
        editar = false;
        cantidad = "";
    }
    ...
    public void seleccionarPedido(Pedido p) throws IOException
    {
        try
        {
            ...
            FacesContext.getCurrentInstance().getExternalContext().redirect("/delivery24/faces/pedidosnuevos/pedido.xhtml");
        }
        catch(IOException e)
        {
    ...
    {
        FacesContext fc = FacesContext.getCurrentInstance();        
        String titulo = "Pedido Confirmado!!!";
        HttpConnection con = new HttpConnection();
        String json = "{\"registration_ids\": [\"" + pedidoSelected.getToken() + "\"]"
                + "     ,\"data\": {\"message\": \"" + "su pedido se esta preparando." + "\","
                + "\"titulo\": \"" + titulo + "\",\"tipo\": 1}}";
        con.postJson(json);

        pedidoSelected.setEstado(1);
        ejbPedidoFacade.edit(pedidoSelected);
        itemsPedido = null;
        pedidoSelected = null;
        fc.getExternalContext().redirect("/delivery24/faces/pedidosnuevos/pedidosnuevos.xhtml");        
